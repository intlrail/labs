let sketch = function(p) {
  ...
  p.draw = function() {
    ...
    init();
    p.fill(p.random(360), 180, 160, .01);
    for (var i = 0; i < 50; i++) {
      current = update();
      display();
    }
  ...
  function init () {
    ...
    for (var i = 0; i < initial_size; i++) {
      points.push(p.createVector((i / (initial_size - 1)) * p.width - (p.width/2), 150, p.random(-1,1)));
    }
    for(let b = 0; b < 6; b++) {
  ...
  function generate_midpoint (p1, p2, sd) {
    let p3 = p.createVector((p1.x + p2.x) / 2, (p1.y + p2.y) / 2, ((p1.z + p2.z) / 2) * .55 * p.random(.5, 6.5));
    move_nearby(p3, sd);
    return p3;
  }
