let sketch = function(p) {
  ...
  let magnitude = 70;
  let noise_delta = 25;
  let noise_radius = 0.27;

  let cols = ['#aa4', '#fb3', '#ec4', '#dd5', '#ada', '#9bc'];
  ...
  p.draw = function() {
    p.push();
    p.translate(p.randomGaussian(p.width / 4, 200), p.randomGaussian(p.height / 4, 200));
    display();
    p.pop();
    display_crosses();
    display_grid();
  ...
  function display_crosses() {
    for (var i = 0; i < 20; i++) {
      p.push();
      p.translate(p.random(30, p.width - 20), p.random(30, p.height - 20));
      p.line(-5, 0, 5, 0);
      p.line(0, -5, 0, 5);
      p.pop();
    }
