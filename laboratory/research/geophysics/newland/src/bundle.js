...
const SVG = opt => {
    ...
    ///////////////////////////////////////////////////////////////////////
    const initSVG = opt => {
        if (opt.centerOrigin !== undefined && opt.centerOrigin === true) offset = 0;
        if (opt.cpuTime !== undefined) cpuTime = opt.cpuTime;
        const svgElem = document.querySelector("svg");
        svgElem.setAttribute("viewBox", opt.viewBox || "0 0 800 600");
        svgElem.setAttribute("width", `${opt.size || 800}px`);
        svgElem.setAttribute("height", `${opt.size || 600}px`);
