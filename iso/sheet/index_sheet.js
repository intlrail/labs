  ...
  var intlrail = [
{
  ...
  let sketch = function(p) {
    ...
    const top = [];
    for (let i = 0; i < 10 * 2 + 11; i++) {
      top.push({ h: false, v: true, in: true, col: '#111' });
    }

    const left = [];
    for (let i = 0; i < 10 * 2 + 11; i++) {
      left.push({ h: true, v: false, in: true, col: '#111' });
    }

    const generator = new index(100, 100, {
      simple: false,
      extension_chance: 0.61,
      horizontal_symmetry: true,
      vertical_chance: 0.34,
      color_mode: 'group',
      initial_top: top,
      initial_left: left
    });
    ...
    p.setup = function() {
      ...
      // p.frameRate(1);
      p.noLoop();
      p.stroke(palette.stroke ? palette.stroke : '#aaa');
    ...
    function displayBox(box, maxLevel, colorize) {
      ...
      p.strokeWeight(0.618 / (box.level + 1));
      p.beginShape();
      ...
      for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
          ...
          const content = apparatus.map(app => ({
            ...
            level: 2,
            filled: true,
            crossed: app.w < 2 && app.h < 2 && Math.random() < 0.9,
