  ...
  var intlrail = [
{
  ...
  let sketch = function (p) {
    ...
    p.setup = function () {
      ...
      p.smooth();
      p.noLoop();
      p.stroke(palette.stroke ? palette.stroke : '#ddd');
    ...
    p.draw = function () {
      if (tick % 9 == 0) reset();
      displayLayout(tick % 10, tick % 10 > 2);
      tick++;
    };

    p.keyPressed = function () {
      if (p.keyCode === 80) p.saveCanvas('isosheet_' + THE_SEED, 'png');
    };
    ...
    function displayLayout(depth, colorize) {
      p.translate(innerWidth*0.2, innerHeight*0.5);
      layout.forEach((box) => {
