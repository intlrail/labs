...
//* canvsa perspective *//
  ...
  quads.forEach(function(q, i) {
    ...
    if (Math.random() > (1 - distance(cq, c) / c.radius) * 2.5) return;

    var offset = 4;

//* skyscraper density *//

    if (q.area < Math.pow(6, 3)) {
...
//*pencil skyscrapers*//
  ...
  big.forEach(function(q) {
    ctx.globalAlpha = .382;
    //ctx.globalAlpha = .125;
    ctx.fillStyle = "rgba(255,66,62,0.5)";
    q.draw(ctx);
    ctx.shadowBlur = 10;
...
//*skyscraper height*//
    ...
    var inc = 1;
    var tot = 20 + Math.random() * 14 * 6;

    ctx.save();
    for (var i = 0; i < tot; i += inc) {
      ctx.translate(-inc, -inc);
      // ctx.globalAlpha = Math.sqrt( ( ( i/tot ) * .15 ) ) //* .5;
      ctx.globalAlpha = Math.pow(i / tot * 1, 0.5);
...
//* Noise FX *//
 ...
 function noise(amount, data, w, h) {
    for (var y = 0; y < h; y++) {
      for (var x = 0; x < w; x++) {
        var id = (y * w + x) * 4;
        var noise = ~~((Math.random() - 0.618) * amount);
        data[id] += noise;
        data[id + 1] += noise;
        data[id + 2] += noise;
      }
