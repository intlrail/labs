...
var intlrail = [{
    ...
    {
        name: 'intlrail2',
        colors: ['#101010', '#1b1b1b', '#242424', '#2c2c2c', '#333333', '#3a3a3a', '#414141', '#4f4f4f', '#565656', '#6b6b6b', '#797979', '#818181', '#919191', '#999999', '#ababab', '#b4b4b4', '#bebebe', '#c8c8c8', '#d2d2d2', '#dddddd', '#e8e8e8', '#f3f3f3'],
        stroke: '#ffffff',
        // background: '#000000',
    }
];
  ...
  function ui (opts, full_reset, redraw, print) {
    ...
    f0.add(opts, 'outerSize', 0, 100, 1).name('Section Sizes').onChange(full_reset);
    f0.add(opts, 'minGridSize', 0, 100, 1).name('Min Grid Size').onChange(full_reset);
    f0.add(opts, 'innerSize', 0, 100, 1).name('Atom Sizes').onChange(full_reset);
    f0.add(opts, 'colorMode', ['single', 'main', 'group', 'random'])
      .name('Color Distr.')
      .onChange(full_reset);
    const f1 = gui.addFolder('Stylistic Changes');
    f1.open();
    f1.add(opts, 'tx', -800, 800, 1).name('Translate X').onChange(redraw);
    f1.add(opts, 'ty', -800, 800, 1).name('Translate Y').onChange(redraw);
    f1.add(opts, 'mag', 1, 100, 0.5).name('Magintude').onChange(redraw);
    f1.add(opts, 'depthDim', 0, 100, 0.1).name('Depth').onChange(redraw);
    f1.add(opts, 'perspective', 0, 100, 0.01).name('Perspective').onChange(redraw);
    const shiftController = f1
      .add(opts, 'paletteShift', 0, 100, 1)
      .listen()
      .name('Palette Shift')
      .onChange(redraw);
  ...
  let sketch = function (p) {
    ...
    function displayLayout() {
      ...
      // p.clear();
      const pal = p.shuffle(palette.colors).slice(0, 5);
    ...
    function displayBox(box, xu, yu, zu, shades, hiddenTop, hiddenLeft, t1, t2, t3, pal) {
      display(
        p,
        box,
        xu,
    ...
    function print() {
      p.saveCanvas('cubetile_' + THE_SEED, 'png');
    }
