...
const SVG = opt => {
    ...
    const initSVG = opt => {
        ...
        svgElem.setAttribute("style", `background:${opt.background || "#000"}`);
        svgElem.setAttribute("viewport-fill", opt.background || "#000");
        const rect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
        rect.setAttribute("style", `fill:${opt.background || "#000"};stroke:none`);
        rect.setAttribute("fill", opt.background || "#000");
        ...
        inside.setAttribute(
            "style",
            `fill:none;stroke:${opt.stroke || "#fff"};stroke-width:${opt.strokeWidth ||
        ...
        outside.setAttribute(
            "style",
            `fill:none;stroke:${opt.stroke || "#fff"};stroke-width:0.1;`
        );
        ...
        save.setAttribute("style", `fill:${opt.background || "#000"};stroke:none;opacity: 0`);
        svgElem.appendChild(save);
        const saveText = document.createElementNS("http://www.w3.org/2000/svg", "g");
        saveText.setAttribute("style", `fill:none;stroke:${opt.stroke || "#fff"};stroke-width:0.1;opacity:1`);
            ...
            for (let i = 0, l = this.dp.length; i < l; i += 2) {
                ...
                if (int.length === 0) {
                    ...
                    for (let i = 0, len = int.length; i < len; i++) {
                        ...
                        const item = int[j];
                        for (
                            const db = (item[0] - ls0[0]) * cx + (item[1] - ls0[1]) * cy; j > 0 &&
                            (int[j - 1][0] - ls0[0]) * cx + (int[j - 1][1] - ls0[1]) * cy < db; j--
                        ) int[j] = int[j - 1];
                        int[j] = item;
                    }
